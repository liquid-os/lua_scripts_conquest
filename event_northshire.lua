...
function On_Gossip(event, plr, unit)
	plr:GossipMenuAddItem(2, "I want to enter the Northshire Invasion event.", 0, 1, false, "This action will cost "..EVENT_COST.." x "..GetItemLink(EVENT_CURRENCY)..". Click accept to confirm.")
...
function On_Select(event, player, unit, sender, intid, code)
	if(intid == 1)then
		if(CURRENT_EVENT == EVENT_NORTHSHIRE or HOME == true)then
...
RegisterCreatureGossipEvent(50178, 1, On_Gossip)
RegisterCreatureGossipEvent(50178, 2, On_Select)
RegisterCreatureEvent(50178, 5, onRespawn)
