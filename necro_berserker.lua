...
function spawn(event, creature)
	...
	creature:SetMaxHealth((creature:GetOwner():GetStat(2) * 1.5 +  creature:GetOwner():GetStat(3) / 3 * 2) * 2.5)
	creature:SetHealth(creature:GetMaxHealth())
	--creature:RegisterEvent(necro_basic_activate, 100, 1)
	initPetAttack(creature)
	creature:RegisterEvent(necro_berserker_infect, 2000, dur / 2000)
...
function necro_berserker_infect(i, ii, iii, creature)
	local owner = creature:GetOwner()
	...
	if(tar ~= nil)then
		creature:CastSpell(tar, 54350, true)
		local owner = creature:GetOwner()
		local dmg = (5 + math.random(5) + owner:GetLevel() * 3 + owner:GetStat(3) / 2) / 2.25
